    ...
    <div className="bg-white">
      <section className="relative overflow-hidden bg-black text-white">
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-10 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-2">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/30 bg-white/10 px-3 py-1.5 text-xs font-bold uppercase tracking-wide text-white">
            ...
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-gray-100">
              ...
              Choose your residence, request a quote, and we will help you plan the details.
            </p>
            <div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 text-sm text-gray-100">
              ...
              <span className="flex items-center gap-2"><ShieldCheck size={16} className="text-white" /> Licensed and insured</span>
              <span className="flex items-center gap-2"><Truck size={16} className="text-white" /> Local moving team</span>
            </div>
            <a
              href={COMPANY.phoneHref}
              onClick={() => pushPhoneClickEvent("seattle-house-hero")}
              className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 font-extrabold text-black shadow-lg transition-colors hover:bg-gray-100"
            >
          ...
          <div id="quote-form" className="rounded-2xl bg-white p-5 shadow-2xl sm:p-7">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-black">Seattle House resident offer</p>
            <h2 className="mt-1 text-2xl font-extrabold text-black">Claim Your Resident Discount</h2>
            <p className="mt-1 text-sm text-gray-600">Select your residence and get a free moving quote.</p>
            <div className="mt-4">
              <QuoteForm
                variant="inline"
      ...
      <section className="border-b border-gray-100 bg-white py-6">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-x-7 gap-y-3 px-4 text-sm text-gray-600">
          <span className="font-semibold text-black">A simple move-in process</span>
          <span>Choose your residence</span>
      ...
      <section className="bg-[#F7F7F5] py-14">
        <div className="mx-auto max-w-5xl px-4 sm:px-6">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-sm font-bold uppercase tracking-[0.14em] text-black">Plan your move day</p>
            <h2 className="mt-2 text-3xl font-extrabold text-black">Typical Moving Time by Home Size</h2>
          ...
          <div className="mt-9 grid gap-5 sm:grid-cols-3">
            {MOVE_TIME_RANGES.map(({ homeSize, range }) => (
              <div key={homeSize} className="rounded-2xl border border-gray-200 bg-white p-6 text-center shadow-sm">
                <Clock3 className="mx-auto text-black" size={28} />
                <h3 className="mt-3 text-lg font-bold text-black">{homeSize}</h3>
                <p className="mt-2 text-2xl font-extrabold text-black">{range}</p>
      ...
      <section className="py-14">
        <div className="mx-auto grid max-w-5xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
          ...
          />
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.14em] text-black">Make move day easier</p>
            <h2 className="mt-2 text-3xl font-extrabold text-black">How to Prepare for Your Move</h2>
            <ul className="mt-6 space-y-4 text-gray-700">
              {[
                "Request your quote early and choose Puget or Cascade in the form.",
                ...
                <li key={item} className="flex gap-3"><CheckCircle2 className="mt-0.5 flex-shrink-0 text-black" size={19} /><span>{item}</span></li>
              ))}
            </ul>
          </div>
        </div>
